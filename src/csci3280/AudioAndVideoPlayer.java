 ...
 * @author aclarke
 */
public class AudioAndVideoPlayer {
    ...
    private static long systemClockPausedTime;
    ...
    public static void setIsPlaying(boolean isPlaying) {
        AudioAndVideoPlayer.isPlaying = isPlaying;
    }
     ...
     *
     * @param filename            filename of the media file to play
     * @param imageView
     * @param sliderVideoProgress
     * @param homeController
     */
    @SuppressWarnings("deprecation")
    public static void playFile(String filename, ImageView imageView, Slider sliderVideoProgress, HomeController homeController) throws InterruptedException {

        //initialize status
        isPlaying = true;
        shouldStop = false;
        shouldPause = false;
        systemClockPausedTime = 0;
     ...
     */
       ...
       */
        ...
        //handle slider
        double videoTimeBase = videoCoder.getStream().getTimeBase().getValue();
        double maxProgress = videoCoder.getStream().getDuration() / videoTimeBase;
        double minProgress = 0;

        sliderVideoProgress.setMin(minProgress);
        sliderVideoProgress.setMax(maxProgress);
       ...
       */
         ...
         */
           ...
           */
                    ...
                    millisecondsUntilTimeToDisplay(newPic);
                    // And finally, convert the picture to an image and display it

                    // mScreen.setImage(Utils.videoPictureToImage(newPic));

                    //count video frame for audio sync
                    videoFrameCount++;
                    //update slider
                    double currentProgress = videoFrameCount / videoTimeBase;

                    final Image jfxImage = SwingFXUtils.toFXImage(Utils.videoPictureToImage(newPic), null);
                    Platform.runLater(new Runnable() {
                        @Override
                        public void run() {
                            imageView.setImage(jfxImage);
                            sliderVideoProgress.setValue(currentProgress);
                        }
                    });
         ...
         */
           ...
           */
                    if (samples.isComplete()) {
                        ...
                        // on another thread.
                        //System.out.println(audioCoder.getStream().getStartTime());
                        //if (videoFrameCount>audioCoder.getStream().getStartTime()) {
     ...
     */
        ...
        Platform.runLater(new Runnable() {
            ...
            public void run() {
                ...
                homeController.resetButtonState();
            }
        });
        closeJavaSound();
        closeJavaVideo();
    ...
    private static long millisecondsUntilTimeToDisplay(IVideoPicture picture) throws InterruptedException {
        /**
        ...
        if (mFirstVideoTimestampInStream == Global.NO_PTS) {
            ...
            long millisecondsClockTimeSinceStartofVideo = systemClockCurrentTime - mSystemVideoClockStartTime - systemClockPausedTime;
            // compute how long for this frame since the first frame in the stream.
            // remember that IVideoPicture and IAudioSamples timestamps are always in MICROSECONDS,
            // so we divide by 1000 to get milliseconds.
            long millisecondsStreamTimeSinceStartOfVideo = (picture.getTimeStamp() - mFirstVideoTimestampInStream) / 1000;
            final long millisecondsTolerance = 1; // and we give ourselfs 50 ms of tolerance
            millisecondsToSleep = (millisecondsStreamTimeSinceStartOfVideo -
                    (millisecondsClockTimeSinceStartofVideo + millisecondsTolerance));
            while (millisecondsStreamTimeSinceStartOfVideo > (millisecondsClockTimeSinceStartofVideo + millisecondsTolerance)) {
                Thread.sleep(1);
                systemClockCurrentTime = System.currentTimeMillis();
                millisecondsClockTimeSinceStartofVideo = systemClockCurrentTime - mSystemVideoClockStartTime - systemClockPausedTime;
            }
        }
    ...
    public static void stopPlaying(boolean stop) {
        AudioAndVideoPlayer.shouldStop = stop;
    }
